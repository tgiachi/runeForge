...
public static class ScriptDescriptorScanner
{
    ...
    )
    {
        ...
        foreach (var param in parameters)
        {
            var paramDescriptor = new ScriptFunctionParameterDescriptor(
                ...
                ParameterType: GetFriendlyTypeName(param.ParameterType),
                RawParameterType: param.ParameterType,
                ParameterTypeString: param.ParameterType.ToString()
            );
    ...
    /// <summary>
    /// Convenience method to scan classes and generate both descriptors and enum types
    /// </summary>
    /// <param name="classTypes">Array of class types to scan</param>
    /// <returns>Tuple containing the descriptors and enum types</returns>
    public static (List<ScriptFunctionDescriptor> Descriptors, List<Type> EnumTypes) ScanClassesWithEnums(
        params Type[] classTypes
    )
    {
        var descriptors = ScanClasses(classTypes);
        var enumTypes = ExtractEnumTypes(classTypes);
        return (descriptors, enumTypes);
    }
}
