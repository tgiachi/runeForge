...
public static class EmmyLuaDefinitionGenerator
{
    ...
    public static string GenerateDefinitions(
        List<ScriptFunctionDescriptor> descriptors, string version, List<Type>? enums = null, List<Type>? customTypes = null
    )
    {
        ...
        sb.AppendLine("---@meta");
        sb.AppendLine();
        sb.AppendLine($"-- Auto-generated EmmyLua definitions for Runeforge {version} script modules");
    ...
    )
    {
        ...
        var hasVariadicParam = function.Parameters.Count > 0 &&
                               function.Parameters.Last().ParameterName.StartsWith("...");

        // Add parameter annotations
        foreach (var param in function.Parameters)
        {
            var paramName = param.ParameterName;
            var isVariadic = paramName.StartsWith("...");

            if (isVariadic)
            {
                // Remove the "..." prefix for the annotation but keep the type
                paramName = paramName[3..]; // Remove "..." prefix
            }
            ...
            sb.AppendLine($"---@param {paramName} {luaType}");
        }
        ...
        foreach (var param in function.Parameters)
        {
            ...
            {
                // For variadic parameters, just use "..." without a name
                paramNames.Add("...");
            }
            else
            {
        ...
        sb.AppendLine($"function {moduleName}.{function.FunctionName}({paramString}) end");
        sb.AppendLine();
    }
    ...
    public static void GenerateDefinitionFile(
        List<ScriptFunctionDescriptor> descriptors, string outputPath, string version, List<Type>? enums = null,
    ...
    )
    {
        ...
        var content = GenerateDefinitions(descriptors, version, enums, customTypes);
    ...
    public static void GenerateModuleDefinitionFiles(
        List<ScriptFunctionDescriptor> descriptors, string outputDirectory, string version, List<Type>? enums = null,
        List<Type>? customTypes = null
    )
    {
        ArgumentNullException.ThrowIfNull(descriptors);
        ...
        foreach (var moduleGroup in moduleGroups)
        {
            ...
            GenerateDefinitionFile(moduleDescriptors, filePath, version, enums, customTypes);
        }
    }
    ...
    public static void GenerateCompleteDefinitionsFromClasses(string outputPath, string version, params Type[] classTypes)
    {
        ArgumentException.ThrowIfNullOrEmpty(outputPath);
        ...
        GenerateDefinitionFile(descriptors, outputPath, version, enumTypes, customTypes);
    }
}
